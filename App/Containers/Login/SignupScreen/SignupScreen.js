    ...
    return (
      <TouchableOpacity onPress={this.props.onPress}>
        <Image
          source={{
            uri:
              'https://www.wearegurgaon.com/wp-content/uploads/2017/09/Tandoori-Momos-Nazims-Gurgaon.jpg',
          }}
  ...
  momoCounter(name) {
    ...
    if (currentState.selectedMomos.includes(name)) {
      ...
      if (currentState.momoCount >= 3) {
        alert(
          'Only upto 3 momos can be selected, choose a biryani instead using our BiryaniNow App'
        );
      } else {
      ...
      <FlatList
        ...
        renderItem={({ item, key }) => (
          <MomoObject
            id={item.id}
            name={item.name}
            onPress={() => this.momoCounter(item.id)}
            style={
              this.state.selectedMomos.includes(item.id) ? styles.momoPicSelected : styles.momoPic
            }
          />
        )}
      />
        ...
        <Content>
          <View style={{ marginBottom: 30 }}>
            <Text style={{ color: 'teal', fontWeight: 'bold', textAlign: 'justify' }}>
              Hi, we just your need name and email to get you started and some preferences and then
              we'll take you to a fascinating world of momos
            </Text>
          </View>
          <View>
            <TextInput
          ...
          <View>
            <Text style={{ color: '#B71C1C', marginTop: 25, textAlign: 'justify' }}>
              Choose upto 3 types of Momos you like to eat, often. We'll get you great deals on
              them!
            </Text>
          </View>

          <Grid>{this.renderMomoList()}</Grid>

          <Button
            style={styles.nextButton}
            disabled={!this.state.flag}
            block
            onPress={() => this.goToAddressChooser()}
          >
